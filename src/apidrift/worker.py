...
def _run_worker(root: str, fqnames: list[str], timeout: float) -> dict[str, IntrospectionRecord]:
    ...
    except Exception:  # deliberate fail-safe: any spawn/pipe/OS error => all unverifiable
        return {}
    finally:
...
def _worker_main() -> int:
    ...
    payload = {fqname: record_to_dict(_introspect_one(root, fqname)) for fqname in fqnames}
    try:
